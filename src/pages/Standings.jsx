...
import { lastNamesOnly, getWeekSide, buildSeedMap, buildPlayoffSeedMap, buildStandingsForSeed, recordPoints, buildHistoricalPlayers, isIndivGroupMatch, currentPlayoffRoundIdx, orderByBracketIdx } from "../lib/league";
...
//  MAIN STANDINGS VIEW
// ════════════════════════════════════════════════════════════
// Thin data wrapper — the board itself is shared with Scoring's trophy popup
// (components/IndividualLeaderboard.jsx). Standings doesn't hold App's live
// hole scores, so the hook opens one subscription for the single unlocked
// playoff week and reads the finalized ones from the cached season tier.
function IndividualBoard({ players, teams, schedule, course, leagueConfig, scoringRules, fetchSeasonScores, fetchAllScores }) {
  const { scores, allRounds, loading } = useIndividualScores({
  ...
  return (
    <IndividualLeaderboard
      ...
      scores={scores} allRounds={allRounds} loading={loading}
    />
  );
}
...
export default function StandingsView({ teams, players, matchResults, leagueConfig, schedule, fetchSeasonScores, course, fetchWeekScores, scoringRules, fetchAllScores, saveMatchResult, dataLoaded, leagueUser, isComm, funRounds, saveFunRound, deleteFunRound, funScores, saveFunScores, appToast, setPopupOpen, season }) {
  ...
  return (
    <div style={{ padding: "0 2px" }}>
      ...
      {view === "individual" && (
        playoffsStarted ? (
          <IndividualBoard players={players} teams={teams} schedule={schedule} course={course} leagueConfig={leagueConfig} scoringRules={scoringRules} fetchSeasonScores={fetchSeasonScores} fetchAllScores={fetchAllScores} />
        ) : (() => {
